/* ============================================================
 * 大鹏数据大屏 DaPengScreen2 · main-red.js（红色主题）
...
if (trendChart) {
  trendChart.setOption({
    ...
    legend: { data: ["今日", "昨日"], textStyle: { color: "#e8b0b0" }, top: 0, right: 10 },
    grid: { left: 40, right: 16, top: 36, bottom: 24 },
    xAxis: { type: "category", data: mock.trend.hours, axisLine: { lineStyle: { color: "#7a2e2e" } },
      axisLabel: { color: "#e8b0b0", interval: 3 } },
    yAxis: { type: "value", splitLine: { lineStyle: { color: "rgba(255,255,255,.06)" } }, axisLabel: { color: "#e8b0b0" } },
    series: [
      { name: "今日", type: "line", smooth: true, symbol: "none", data: mock.trend.today,
        lineStyle: { color: "#ff4d4f", width: 2 },
        areaStyle: { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: "rgba(255,77,79,.45)" }, { offset: 1, color: "rgba(255,77,79,0)" }]) } },
      { name: "昨日", type: "line", smooth: true, symbol: "none", data: mock.trend.yest,
        lineStyle: { color: "#ff85a2", width: 2 },
        areaStyle: { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: "rgba(255,133,162,.30)" }, { offset: 1, color: "rgba(255,133,162,0)" }]) } },
    ],
  });
  setInterval(() => {
...
if (categoryChart) {
  const palette = ["#ff4d4f", "#ff7a45", "#ffc53d", "#ff85a2", "#cf1322"];
  categoryChart.setOption({
    color: palette,
    tooltip: { trigger: "item", formatter: "{b}: {c}%" },
    legend: { bottom: 0, textStyle: { color: "#e8b0b0" } },
    series: [{
      type: "pie", radius: ["45%", "70%"], center: ["50%", "45%"], avoidLabelOverlap: false,
      itemStyle: { borderColor: "#2a0608", borderWidth: 2 },
      label: { color: "#ffd9d9", formatter: "{b}\n{c}%" },
      data: mock.category,
    }],
  });
  setInterval(() => {
...
if (channelChart) {
  channelChart.setOption({
    ...
    xAxis: { type: "value", axisLabel: { color: "#e8b0b0" }, splitLine: { lineStyle: { color: "rgba(255,255,255,.06)" } } },
    yAxis: { type: "category", data: mock.channel.map((c) => c.name), axisLine: { lineStyle: { color: "#7a2e2e" } }, axisLabel: { color: "#ffd9d9" } },
    series: [{
      type: "bar", data: mock.channel.map((c) => c.value), barWidth: 14,
      label: { show: true, position: "right", color: "#ffd9d9", formatter: "{c}%" },
      itemStyle: { borderRadius: 7, color: new echarts.graphic.LinearGradient(0, 0, 1, 0, [{ offset: 0, color: "#ff4d4f" }, { offset: 1, color: "#ff7a45" }]) },
    }],
  });
  setInterval(() => {
...
function gaugeOpt(name, val, color) {
  return {
    series: [{
      ...
      axisTick: { show: false }, splitLine: { length: 8, lineStyle: { color: "#7a2e2e" } },
      axisLabel: { color: "#e0a3a3", fontSize: 10, distance: 12 },
      pointer: { width: 4, itemStyle: { color } },
      anchor: { show: true, size: 8, itemStyle: { color } },
      title: { offsetCenter: [0, "32%"], color: "#ffd9d9", fontSize: 13 },
...
if (g1) g1.setOption(gaugeOpt(mock.gauges[0].name, mock.gauges[0].val, "#ff4d4f"));
if (g2) g2.setOption(gaugeOpt(mock.gauges[1].name, mock.gauges[1].val, "#ffc53d"));
setInterval(() => {
...
if (mapChart) {
  mapChart.showLoading({ text: "地图加载中...", textColor: "#ff4d4f", maskColor: "rgba(26,3,6,.6)" });
    ...
    .then((geo) => {
      ...
      mapChart.setOption({
        ...
        visualMap: { min: 100, max: 1000, left: 16, bottom: 16, calculable: true,
          textStyle: { color: "#e8b0b0" }, inRange: { color: ["#3a0a0f", "#a8071a", "#ff4d4f"] } },
        geo: { map: "china", roam: false, label: { show: false },
          itemStyle: { areaColor: "rgba(71,13,13,.6)", borderColor: "rgba(255,77,79,.4)" },
          emphasis: { itemStyle: { areaColor: "rgba(255,77,79,.25)" }, label: { color: "#fff" } } },
        series: [
          ...
          { name: "飞线", type: "lines", coordinateSystem: "geo", zlevel: 2,
            effect: { show: true, period: 5, trailLength: 0.4, color: "#ffc53d", symbol: "arrow", symbolSize: 6 },
            lineStyle: { color: "#ffc53d", width: 1, opacity: 0.6, curveness: 0.3 }, data: lines },
          { name: "城市", type: "effectScatter", coordinateSystem: "geo", zlevel: 3,
            rippleEffect: { brushType: "stroke", scale: 4 }, symbolSize: (v) => 6 + v[2] / 90,
            itemStyle: { color: "#ffc53d" }, data: mock.cities.map((c) => ({ name: c.name, value: c.coord.concat(c.value) })) },
        ],
      });
      setInterval(() => {
